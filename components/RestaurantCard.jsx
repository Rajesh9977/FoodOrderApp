...
const RestaurantCard = ({
  ...
  const imageUrl = `https://media-assets.swiggy.com/swiggy/image/upload/w_256,h_192,c_fill/${cloudinaryImageId}`;
  const cuisineText = cuisines?.join(", ");
  const deliveryTime = sla?.deliveryTime ? `${sla.deliveryTime} mins` : '';
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm p-3 w-full max-w-[180px] hover:shadow-md transition text-sm">
      ...
      <div className="text-gray-500 text-xs truncate">{cuisineText}</div>
